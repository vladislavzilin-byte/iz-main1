...
export default function Auth({ onAuth }) {
  ...
  const submit = (e) => {
    ...
    if (mode === 'register') {
      ...
      const user = { phone, name, password, instagram }
      users.push(user)
      saveUsers(users)
      setCurrentUser(user)
      onAuth?.(user)
    ...
    return (
      <div className="card">
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', gap:12}}>
          <div>
            <div>Вошли как <b>{current.name}</b> <small className="muted">({current.phone})</small></div>
            {current.instagram && <div><small className="muted">Instagram: {current.instagram}</small></div>}
            <small className="muted">Нажмите «Выйти», чтобы сменить аккаунт.</small>
          </div>
      ...
      <form onSubmit={submit} className="row">
        ...
        {mode==='register' && (
          <div className="col">
            ...
            <input value={instagram} onChange={e=>setInstagram(e.target.value)} placeholder="@iz_hair_trend" />
          </div>
        )}
        <div className="col">
          <label>Телефон</label>
